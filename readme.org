...
To interpret a free monad to other monads, we have the ~foldFree~ function, which is essentially a catamorphism:
...
Similar to the trivial first order language before, our calculator DSL only has integers, addition, and I/O. The underlying functor is defined as: (note the continuations ~(X -> r)~ presumably can be eliminated using *freer monads*)
...
Then we define the following term constructors, this boilerplate should be automated using ~makeFree~ in the Haskell library: https://hackage.haskell.org/package/free-5.2/docs/Control-Monad-Free-TH.html.
...
The program now is constructed in a monadic style, which provides some syntactic convenience. However, this forces the programmer to choose the evaluation order and linearize the expression AST. But sometimes we want to have access to the tree representation, eg. pretty printing.
...
The interpreter is written as above. Our STLC is implemented likewise, we omit the code here since it's not noteworthy.
